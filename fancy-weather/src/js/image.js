...
export default async function getImage(month, hour,icon) {
	...
	try{
	sessionStorage.setItem('weather', `${month},${hour}`);
	...
	preloaderImg.addEventListener('load', () => {
		...
		backgroundImage.style.backgroundImage = `url(${data.urls.regular}),url(./image/background-image.jpg)`;
		preloaderImg = null;
	});
} catch {
